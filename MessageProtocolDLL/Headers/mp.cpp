...
int MessageProtocol::requesthelper::request_reader(ACE_SOCK_Stream& _peer, std::string & container, int len) {
...
void MessageProtocol::responsehelper::getlength(char ** buffer, int length) {
	std::stringstream ss;
	ss << std::setw(4) << std::setfill('0') << length;
	*buffer = _strdup(ss.str().c_str());
}

std::string MessageProtocol::responsehelper::parseresponse(response &resp, std::string payload, bool string) {
	std::stringstream ss;
